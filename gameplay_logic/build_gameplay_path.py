"""Build a reusable gameplay path from fixed gameplay placements.
...
DEFAULT_OUTPUT = os.path.join(STAGING_ROOT, "gameplay_path", "gameplay_path.json")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build a reusable gameplay path from fixed placements")
...
def build_director(placement_data, asset_data, args):
    ...
    return {
        "schema_version": 3, "status": "gameplay_path_from_fixed_placements",
        "stage": "gameplay_path", "genre": args.genre,
...
def main():
    ...
    print("GAMEPLAY_PATH", os.path.abspath(args.output))
    print("GAMEPLAY_BEATS", len(output["beats"]))
    print("GAMEPLAY_PATH_LENGTH_M", output["estimated_actor_path_length_m"])
    print("GAMEPLAY_FRAME_RANGE", output["frame_start"], output["frame_end"])
